...
注意：即使判断为不重要（important: false），也要填写title、summary、category字段（可以简单填写），并在 passReason 中写明原因。`;
    ...
    try {
      ...
      console.error('Claude processNews error:', error);
      throw error;
    }
  }
...
${content}`;
    ...
    try {
      ...
      console.error('Claude translateContent error:', error);
      throw error;
    }
  }
  ...
  async summarizeLongform(input: LongformDigestInput): Promise<LongformDigestDraft> {
    try {
      ...
      console.error('Claude summarizeLongform error:', error);
      throw error;
    }
  }
...
请只返回简介文字：`;
    ...
    try {
      ...
      console.error('Claude summarizeAuthorBio error:', error);
      // 降级方案：返回简化的原始简介
...
请只返回一个0-100之间的整数，不要任何其他文字。`;
    ...
    try {
      ...
      console.error('Claude scoreNewsImportance error:', error);
...
请严格按照以下JSON格式返回（不要包含任何其他文字）：
{
  ...
  "eventType": "announcement"
    ...
    try {
      ...
      console.error('Claude generateSemanticFingerprint error:', error);
      // 降级方案：返回基础指纹
      return {
...
请严格按照以下JSON格式返回（不要包含任何其他文字）：
[
  {"id1": "...", "id2": "...", "similarity": 85, "reason": "都在讨论 Yann LeCun 发布的世界模型论文"},
  ...
]
...
只返回相似度 >= 60 的配对。如果没有相似的配对，返回空数组 []。`;
    ...
    try {
      ...
      console.error('Claude compareSimilarityBatch error:', error);
      // 降级方案：返回空数组（不进行语义去重）
      return [];
    }
  }
...
${ref ? '  "translatedTextReferenced": "...",\n' : ''}  "highlights": ["要点一", "要点二"],
  ...
  "noveltyScore": 90
    ...
    try {
      ...
      console.error('Claude analyzePost error:', error);
      // 降级方案：返回基本信息
      return {
        canonicalSummary: '',
        entities: [],
